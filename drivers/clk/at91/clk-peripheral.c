						...
						struct clk_rate_request *req)
{
	...
	/* Step two: check to request rate from parent. */
				    ...
				    int chg_pid)
{
	...
	init.name = name;
	init.ops = chg_pid < 0 ? &sam9x5_peripheral_ops : &sam9x5_peripheral_chg_ops;
	init.parent_names = &parent_name;
	init.num_parents = 1;
	init.flags = chg_pid < 0 ? 0 : CLK_SET_RATE_GATE | CLK_SET_PARENT_GATE |
				       CLK_SET_RATE_PARENT;

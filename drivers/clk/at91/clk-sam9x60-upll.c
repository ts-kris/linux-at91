...
#define PLL_STATUS_MASK(id)	(1 << (id))
#define PLL_REG(id)		(AT91_PMC_PLL_CTRL0)
#define PLL_REG1(id)	(AT91_PMC_PLL_CTRL1)
#define PLL_DIV_MASK		0xff
#define PLL_DIV_MAX		(PLL_DIV_MASK + 1)
#define PLL_DIV(reg)		((reg) & PLL_DIV_MASK)
#define PLL_MUL(reg, layout)	(((reg) >> (layout)->mul_shift) & \
				 (layout)->mul_mask)
#define PLL_MUL_MIN		1
...
#define PLL_MAX_ID		1
#define PLLA_LOOP_FILTER	0x9 /* in datasheet: "Recommended value for this field = 0x09" */
#define PLLA_FRACR		0
...
static int clk_pll_prepare(struct clk_hw *hw)
{
	...
	//const struct clk_pll_characteristics *characteristics = pll->characteristics;
	u8 id = pll->id;
	u32 mask = PLL_STATUS_MASK(id);
	int offset = PLL_REG(id);
	int offset1 = PLL_REG(id);
	unsigned int pllr;
	unsigned int status;
	u8 div;
	u16 mul;
	...
	status &= ~(AT91_PMC_PLL_UPDT_STUPTIM_MSK | AT91_PMC_PLL_UPDT_UPDATE | AT91_PMC_PLL_UPDT_ID_MSK);
	status |= AT91_PMC_PLL_UPDT_ID(id);
	regmap_write(regmap, AT91_PMC_PLL_UPDT, status);
	...
	pr_debug("%s upll: div = %d, mul = %d\n", __func__, pll->div, pll->mul);
	...
	status &= (~AT91_PMC_PLL_ACR_LOOP_FILTER_MSK);
	status |= AT91_PMC_PLL_ACR_LOOP_FILTER(PLLA_LOOP_FILTER);
	 ...
	 */
	regmap_read(regmap, AT91_PMC_PLL_UPDT, &status);
	status |= AT91_PMC_PLL_UPDT_UPDATE | AT91_PMC_PLL_UPDT_ID(id);
	regmap_write(regmap, AT91_PMC_PLL_UPDT, status);
	 ...
	 */
	regmap_read(regmap, AT91_PMC_PLL_CTRL0, &status);
	status &= ~(AT91_PMC_PLL_CTRL0_DIVPMC_MSK);
	status |= AT91_PMC_PLL_CTRL0_ENLOCK | AT91_PMC_PLL_CTRL0_ENPLL | AT91_PMC_PLL_CTRL0_ENPLLCK;
	if (id != PLL_ID_UPLL) {
		status |= AT91_PMC_PLL_CTRL0_DIVPMC(pll->div);
	}
	regmap_write(regmap, AT91_PMC_PLL_CTRL0, status);
...
static void clk_pll_unprepare(struct clk_hw *hw)
{
	 ...
	 */
	regmap_read(regmap, AT91_PMC_PLL_UPDT, &reg);
	reg &= ~(AT91_PMC_PLL_UPDT_UPDATE | AT91_PMC_PLL_UPDT_ID_MSK);
	reg |= AT91_PMC_PLL_UPDT_ID(pll->id);
	regmap_write(regmap, AT91_PMC_PLL_UPDT, reg);

	/* 3. In PMC_PLL_CTRL0, set ENPLLCK to 0 and leave ENPLL at '1'. */
	regmap_read(regmap, AT91_PMC_PLL_CTRL0, &reg);
	reg &= (~AT91_PMC_PLL_CTRL0_ENPLLCK);
	regmap_write(regmap, AT91_PMC_PLL_CTRL0, (reg | AT91_PMC_PLL_CTRL0_ENPLL));

	/* 4. Set PMC_PLL_UPDT.UPDATE to '1'. PMC_PLL_UPDT.ID must equal the one written during step
	 * 2, else the update is cancelled.
	 */
	regmap_read(regmap, AT91_PMC_PLL_UPDT, &reg);
	reg &= ~(AT91_PMC_PLL_UPDT_ID_MSK);
	reg |= AT91_PMC_PLL_UPDT_ID(pll->id) | AT91_PMC_PLL_UPDT_UPDATE;
	regmap_write(regmap, AT91_PMC_PLL_UPDT, reg);
					 ...
					 unsigned long parent_rate)
{
	unsigned long val;
	struct clk_pll *pll = to_clk_pll(hw);
	val = (parent_rate * (pll->mul + 1)) / (pll->div + 1);
	pr_debug("%s upll: val = %lu\n", __func__, val);
	return val;
}
...
static long clk_pll_get_best_div_mul(struct clk_pll *pll, unsigned long rate,
	...
	pr_debug("%s upll: begin for rate = %lu\n", __func__, rate);

	/* Check if parent_rate is a valid input rate */
	if (parent_rate < characteristics->input.min || parent_rate > characteristics->input.max)
		return -ERANGE;

	/*
	 * Calculate the multiplier that provide the closest rate to the
	 * requested one.
	 */
	tmpmul = DIV_ROUND_CLOSEST(2 * rate, parent_rate);
	bestrate = (parent_rate * tmpmul) / 2;
...
at91_clk_register_upll(struct regmap *regmap, const char *name,
...
static const struct clk_pll_layout sam9x60_pll_layout = {
	.mul_shift = 24,
	.mul_mask = 0x7F,
};
...
of_at91_clk_upll_get_characteristics(struct device_node *np)
{
	int i;
	int offset;
	u32 tmp;
...
static void __init
of_sam9x60_clk_upll_setup(struct device_node *np,
		      const struct clk_pll_layout *layout)
{
	u32 id;
	struct clk_hw *hw;
	...
	characteristics = of_at91_clk_upll_get_characteristics(np);
	if (!characteristics)
		return;

	hw = at91_clk_register_upll(regmap, name, parent_name, id, layout,
				    characteristics);
	if (IS_ERR(hw))
		goto out_free_characteristics;
...
static void __init sam9x60_clk_upll_setup(struct device_node *np)
{
	of_sam9x60_clk_upll_setup(np, &sam9x60_pll_layout);
}
CLK_OF_DECLARE(sam9x60_clk_upll, "microchip,sam9x60-clk-upll",
	       sam9x60_clk_upll_setup);

...
static void __init
of_sam9x60_clk_pll_setup(struct device_node *np,
		      const struct clk_pll_layout *layout)
{
	u32 id;
	struct clk_hw *hw;
...
static void __init sam9x60_clk_pll_setup(struct device_node *np)
{
	of_sam9x60_clk_pll_setup(np, &sam9x60_pll_layout);
}
CLK_OF_DECLARE(sam9x60_clk_pll, "microchip,sam9x60-clk-pll",
	       sam9x60_clk_pll_setup);

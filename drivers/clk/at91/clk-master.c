					     ...
					     struct clk_rate_request *req)
{
	...
	/*
	 * Second: try to request rate form changeable parent and
	 * use MCR dividers.
	 */
	if (master->chg_pid < 0)
		goto end;
				 ...
				 bool fixed, int chg_pid)
{
	...
	if (!name || !num_parents || !parent_names || id > MASTER_MAX_ID)
		return ERR_PTR(-EINVAL);
	...
	init.parent_names = parent_names;
	init.num_parents = num_parents;
	init.flags = fixed ? CLK_IS_CRITICAL : CLK_SET_RATE_GATE |
					       CLK_SET_PARENT_GATE |
					       CLK_SET_RATE_PARENT;

...
static const struct {
	...
	{ .n  = "tcpca_gclk",
	  .id = 94,
	  .r = { .max = 32768, },
	  .pp_chg_id = INT_MIN, }, /* TODO: invalid parents in datasheet */

	{ .n  = "tcpcb_gclk",
	  .id = 95,
	  .r = { .max = 32768, },
	  .pp_chg_id = INT_MIN, }, /* TODO: ditto */
};
...
static void __init sama7g5_pmc_setup(struct device_node *np)
{
	...
	sama7g5_pmc = pmc_data_allocate(PMC_CORE_MAX + 1,
	...
	alloc_mem = kmalloc(sizeof(void *) *
			    (ARRAY_SIZE(sama7g5_gck) + ARRAY_SIZE(sama7g5_gck)),
			    GFP_KERNEL);
	if (!alloc_mem)
		goto err_free;

	/*
	 * TODO: accuracy is TBD in early datasheet. Updated when datasheet
	 * is ready.
	 */
	...
	for (i = 0; i < ARRAY_SIZE(sama7g5_mckX); i++) {
		...
		mux_table = kmalloc(sizeof(u32) * num_parents, GFP_KERNEL);
		if (!mux_table)
			goto err_free;
